...
const TOAST_LIMIT = Number(process.env.NEXT_PUBLIC_TOAST_LIMIT ?? 3)
const TOAST_REMOVE_DELAY = Number(
  process.env.NEXT_PUBLIC_TOAST_REMOVE_DELAY ?? 5000
)
...
// ✅ Ensure we clear pending remove timeouts when needed
...
export const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case 'REMOVE_TOAST': {
      // ✅ Clear any pending timeout for the removed toast(s)
