...
import { register, getBatches, ApiError } from '@/lib/api'
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Welcome!',
        description: `Your account has been created successfully for ${batch}.`,
      })
      ...
      <div className='w-full max-w-md'>
        <div className='mb-6 text-center text-slate-100'>
          ...
          <p className='mt-2 text-sm text-slate-400'>
            Create your account to access the helpdesk forum
          </p>
        </div>
        ...
        >
          ...
          <div className='space-y-1.5'>
            ...
            <Select value={batch} onValueChange={setBatch} disabled={batchesLoading}>
              <SelectTrigger className='bg-slate-900/70 border-slate-700 text-slate-50'>
                <SelectValue placeholder={batchesLoading ? 'Loading batches...' : 'Choose your batch'} />
              </SelectTrigger>
              <SelectContent className='bg-slate-900 border-slate-700'>
                {batches.map(b => (
                  <SelectItem key={b} value={b} className='text-slate-100'>
                    {b}
                  </SelectItem>
                ))}
              </SelectContent>
            ...
            <p className='text-[10px] text-slate-500'>
              You can only create threads in your selected batch
            </p>
          </div>
          ...
          <Button
            type='submit'
            disabled={loading || avatarUploading || batchesLoading || !batch}
